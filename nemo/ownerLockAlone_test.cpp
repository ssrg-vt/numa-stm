...
volatile int owner = 0, owner_in = 0, request = 0, counter = 0;
...
void* th_run(void * args)
{
	...
	while(ExperimentInProgress) {
		...
		while(granted == 0) {
			...
			}else if (__sync_bool_compare_and_swap(&(request), 0, id)) {
				int old_owner = owner;
				owner = id;
				MFENCE;
				while (owner_in != 0 && owner_in == old_owner){
					asm volatile ("pause");
				}
				if (owner_in < 0) {
					flags[id] = 0;
