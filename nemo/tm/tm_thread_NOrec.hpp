...
FORCE_INLINE uintptr_t validate(Tx_Context* tx)
{
	while (true) {
		  // read the lock until it is even
		  uintptr_t s = ts_vectors[0]->val[0];
		  if ((s & 1) == 1)
			  continue;
...
FORCE_INLINE T tm_read(T* addr, Tx_Context* tx, int numa_zone)
{
	...
	T val = *addr;
	while(tx->start_time[0] != ts_vectors[0]->val[0]) {
		if ((tx->start_time[0] = validate(tx)) == VALIDATION_FAILED)
			tm_abort(tx, 0);
		val = *addr;
		CFENCE;
	}
...
#define TM_BEGIN												\
	{															\
		...
		{														\
			...
			tx->writeset->reset();								\
			tx->readset->reset();								\
			tx->start_time[0] = ts_vectors[0]->val[0] & ~(1L);
