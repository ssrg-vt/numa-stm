...
FORCE_INLINE T tm_read(T* addr, Tx_Context* tx, int numa_zone)
{
	...
	if (v1 > tx->start_time[numa_zone]) {
		if (numa_zone != tx->numa_zone) {
			...
			if (!tx->touched_zones[numa_zone] && ts_vectors[numa_zone]->val[tx->numa_zone] <= tx->start_time[tx->numa_zone]) { //must be for all currently touched zones
				tx->mineNotChanged++;
...
FORCE_INLINE void tm_commit(Tx_Context* tx)
{
	...
	for (int i=0; i<count; i++) {
		while (!__sync_bool_compare_and_swap(&(ts_loc[indexes[i]]->val), 0, 1)) pause();
	}
	//CFENCE;
...
#define TM_BEGIN												\
	{															\
		...
		{														\
			...
			tx->writeset->reset();								\
			for (int i=0; i<8; i++) {tx->touched_zones[i] = false;}\
			for (int i=0; i<8; i++) {							\
