...
FORCE_INLINE T tm_read(T* addr, Tx_Context* tx, int numa_zone)
{
	...
	lock_entry* entry_p = &(lock_table[0][index]);

	uint64_t v1 = entry_p->version;
	T val = *addr;
	uint64_t v2 = entry_p->version;
	if (v1 > tx->start_time[0] || (v1 != v2) || entry_p->lock_owner) {
		uintptr_t cur_ts = ts_vectors[0]->val[0];
		if (v1 > cur_ts) {
			__sync_val_compare_and_swap(&ts_vectors[0]->val[0], cur_ts, v1);
		}
		tm_abort(tx, 0);
	}
...
FORCE_INLINE void tm_commit(Tx_Context* tx)
{
	...
	for (int i = 0; i < tx->reads_pos; i++) {
		int srv_addr = 0;//tx->r_zone[i];
		lock_entry* entry_p = &(lock_table[srv_addr][tx->reads[i]]);
		if (entry_p->version > tx->start_time[srv_addr]) {
			if (entry_p->version > max_ts) max_ts = entry_p->version;
			do_abort = true;
		}
	}

	if (do_abort) {
		__sync_val_compare_and_swap(&ts_vectors[0]->val[0], ts_vectors[0]->val[0], max_ts);
		for (int i = 0; i < tx->writes_pos; i++) {
			if (tx->granted_writes[i]) {
				int srv_addr = 0;//tx->w_zone[i];
				lock_entry* entry_p = &(lock_table[srv_addr][tx->writes[i]]);
...
//
//	max_ts++;
	uintptr_t next_ts = ts_vectors[0]->val[0]+1;//__sync_val_compare_and_swap(&ts_vectors[0]->val[0], cur_ts, cur_ts+1);//__sync_fetch_and_add(&ts_vectors[0]->val[0], 1);
	//CFENCE;
/*	for (int i=0; i<count; i++) {
		//for (int j=0; j<count; j++) {
		next_ts[indexes[i]] = __sync_fetch_and_add(&ts_vectors[indexes[i]]->val[indexes[i]], 1);
		//}
	}
*/
...
#define TM_BEGIN												\
	{															\
		...
		{														\
			...
			tx->writeset->reset();								\
			tx->count++;		\
			tx->start_time[0] = ts_vectors[0]->val[0];
