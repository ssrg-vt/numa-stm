...
FORCE_INLINE uintptr_t validate(Tx_Context* tx, bool ignoreLock)
{
	while (true) {
		  // read the lock until it is even
		  uintptr_t s = numa_lock.get_count();
		  if (!ignoreLock && numa_lock.is_locked(tx->numa_zone))
			  continue;
		  ...
		  // validation early
		  bool valid = tx->readset->validate();
...
FORCE_INLINE T tm_read(T* addr, Tx_Context* tx, int numa_zone)
{
	...
	T val = *addr;
	while(tx->start_time[0] != numa_lock.get_count()) {
		if ((tx->start_time[0] = validate(tx, false)) == VALIDATION_FAILED)
			tm_abort(tx, 0);
		val = *addr;
		CFENCE;
	}
	tx->readset->insert(WriteSetEntry((void**)addr, (uint64_t)val, sizeof(T)));
	return val;
}
...
{
//    bool alreadyExists =
    		tx->writeset->insert(WriteSetEntry((void**)addr, (uint64_t)val, size));
...
FORCE_INLINE void thread_init(int id, int numa_zone) {
	if (!Self) {
		...
		tx->readset = new WriteSet(ACCESS_SIZE);
//		printf("tx pointer %d\n", tx);
...
FORCE_INLINE void tm_commit(Tx_Context* tx)
{
	...
	numa_lock.lock(tx->numa_zone);
    //while (!__sync_bool_compare_and_swap(&ts_vectors[0]->val[0], tx->start_time[0], tx->start_time[0] + 1))
	if (numa_lock.get_count() != tx->start_time[0]) {
        if ((tx->start_time[0] = validate(tx, true)) == VALIDATION_FAILED) {
        	numa_lock.unlock(tx->numa_zone, false);
            tm_abort(tx, 0);
        }
	}
    ...
    CFENCE;
    numa_lock.unlock(tx->numa_zone, true);
    //ts_vectors[0]->val[0] = tx->start_time[0] + 2;
...
#define TM_BEGIN												\
	{															\
		...
		{														\
			...
			tx->writeset->reset();								\
			tx->readset->reset();								\
			tx->start_time[0] = numa_lock.get_count();
