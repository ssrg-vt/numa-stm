...
FORCE_INLINE T tm_read(T* addr, Tx_Context* tx, int numa_zone)
{
	...
	if (v1 > tx->start_time[numa_zone] || (v1 != v2) || entry_p->lock_owner) {
...
FORCE_INLINE void tm_commit(Tx_Context* tx)
{
	...
	if (count > 1 || indexes[0] != tx->numa_zone) tx->internuma++;
...
//	max_ts++;
	...
	for (int i=0; i<count; i++) {
		...
		tx->start_time[indexes[i]] = next_ts[indexes[i]];
		//}
	}
...
#define TM_BEGIN												\
	{															\
		...
		{														\
			...
			if (tx->count % 50 == 0) { \
				for (int i=0; i<8; i++) {							\
					tx->start_time[i] = ts_vectors[i]->val[i];\
				}													\
			}\
