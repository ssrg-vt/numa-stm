...
void* th_run(void * args)
{
    ...
    //printf("my id %d and zone %d\n", id, numa_zone);
    	...
    	long* accounts = accountsAll[numa_zone];

    	long* accounts2 = accountsAll[(numa_zone + 1) % 8];
...
//		uint64_t start = tick();
		...
		//	retries++;
			...
			for (int j=0; j< 10; j++) {
				if (tx_count % percentage == 0 && once){
					TM_WRITE_Z(accounts2[acc1[j]], TM_READ_Z(accounts2[acc1[j]], (numa_zone +1)%8) + 50, (numa_zone +1)%8);
					TM_WRITE_Z(accounts2[acc2[j]], TM_READ_Z(accounts2[acc2[j]], (numa_zone +1)%8) - 50, (numa_zone +1)%8);
					once = false;
				} else {
...
int main(int argc, char* argv[])
{
	...
	if (argc < 3) {
		printf("Usage test threads_per_zone# percentage\n");
		exit(0);
	}
