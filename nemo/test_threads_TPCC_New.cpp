...
typedef struct customer{
	 ...
	 int C_DELIVERY_CNT;
	 std::string C_DATA;
	 customer() : C_CREDIT_LIM(50000.0), C_DISCOUNT(10.0), C_YTD_PAYMENT(10.0), C_PAYMENT_CNT(1), C_DELIVERY_CNT(0), C_SINCE(22.2), C_BALANCE(10.0)
	     {
		 	 int s = rand_r_32(&gseed) % 17;
		 	 C_FIRST = std::string(s, 'a');
		 	s = rand_r_32(&gseed) % 17;
		 	C_LAST = std::string(s, 'a');
		 	s = rand_r_32(&gseed) % 21;
		 	C_STREET_1 = std::string(s, 'a');
		 	s = rand_r_32(&gseed) % 21;
		 	C_STREET_2 = std::string(s, 'a');
		 	s = rand_r_32(&gseed) % 21;
		 	C_CITY = std::string(s, 'a');

		 	s = rand_r_32(&gseed) % 500;
		 	C_DATA = std::string(s, 'a');
...
//	uint64_t total = 0;
	...
	while(ExperimentInProgress) {
		...
		//			(TX_NEWORDER);
				...
				int C_LAST = TM_READ_Z(customers[w_id][d_id][c_id].C_LAST, tx_numa);
	...
	//			norder.O_ALL_LOCAL = true;
				...
				for (int i = 0; i <= items_count; i++) {
					...
					int I_NAME = TM_READ_Z(items[i_id].I_NAME, tx_numa);
					int I_DATA = TM_READ_Z(items[i_id].I_DATA, tx_numa);
