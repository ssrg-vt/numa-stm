...
FORCE_INLINE T tm_read(T* addr, Tx_Context* tx, int numa_zone)
{
	...
	if ((v1 != v2) || entry_p->lock_owner) {
		tm_abort(tx, 0);
	}
	if (v1 > tx->start_time[numa_zone]) {
		if (numa_zone != tx->numa_zone) {
			uintptr_t ts = ts_vectors[tx->numa_zone]->val[numa_zone];
			while (ts < v1) {
				uintptr_t ts_old = __sync_val_compare_and_swap(&(ts_vectors[tx->numa_zone]->val[numa_zone]), ts, ts_vectors[numa_zone]->val[numa_zone]);
				if (ts_old > ts) {
					ts = ts_old;
				}
			}
//			if (ts_vectors[tx->numa_zone]->val[numa_zone] < v1) {
//				while (!__sync_bool_compare_and_swap(&(ts_loc[tx->numa_zone]->val), 0, 1)) pause();
//				if (ts_vectors[tx->numa_zone]->val[numa_zone] < v1) {
//					ts_vectors[tx->numa_zone]->val[numa_zone] = ts_vectors[numa_zone]->val[numa_zone];
//				}
//				ts_loc[tx->numa_zone]->val = 0;
//			}
			if (!tx->touched_zones[numa_zone] && ts_vectors[numa_zone]->val[tx->numa_zone] <= tx->start_time[tx->numa_zone]) {
				tx->mineNotChanged++;
				tx->start_time[numa_zone] = ts_vectors[numa_zone]->val[numa_zone];
			} else {
				tm_abort(tx, 0);
			}
			//update local timestamp and zone timestamp and continue
		}
		else
			tm_abort(tx, 0);
	}
...
FORCE_INLINE void tm_commit(Tx_Context* tx)
{
	...
	for (int i = 0; i < tx->reads_pos; i++) {
		...
		//if (srv_addr == tx->numa_zone) {
		lock_entry* entry_p = &(lock_table[srv_addr][tx->reads[i]]);
		if (entry_p->version > tx->start_time[srv_addr]) {
			for (int i = 0; i < tx->writes_pos; i++) {
				if (tx->granted_writes[i]) {
					int srv_addr = tx->w_zone[i];
					lock_entry* entry_p = &(lock_table[srv_addr][tx->writes[i]]);
	...
	//increment my numa zone timestamp
	...
	int count = 0;
	for (int i=0; i<8; i++) {
		if (tx->touched_zones[i] >= 2) {
			indexes[count++] = i;
		}
	}

	if (count > 1) tx->internuma++;



	for (int i=0; i<count; i++) {
		next_ts[indexes[i]] = __sync_fetch_and_add(&ts_vectors[indexes[i]]->val[indexes[i]], 1) + 1;
	}

	//update other entries if possible
	for (int i=0; i<count; i++) {
		for (int j=0; j<count; j++) {
			if (i==j) continue;
			__sync_bool_compare_and_swap(&(ts_vectors[indexes[i]]->val[indexes[j]]), ts_vectors[indexes[i]]->val[indexes[j]], next_ts[indexes[j]]);
		}
	}
...
/*
	//lock them all
	for (int i=0; i<count; i++) {
		while (!__sync_bool_compare_and_swap(&(ts_loc[indexes[i]]->val), 0, 1)) {
			while(ts_loc[indexes[i]]->val) spin64();
		}
	}
	//CFENCE;
	...
	for (int i=0; i<count; i++) {
		for (int j=0; j<count; j++) {
			uintptr_t ts = ts_vectors[indexes[i]]->val[indexes[j]];
			if (ts > max_ts) {
				max_ts = ts;
			}
		}
	...
	for (int i=0; i<count; i++) {
		for (int j=0; j<count; j++) {
			ts_vectors[indexes[i]]->val[indexes[j]] = max_ts;
		}
	}

	//unlock
	for (int i=0; i<count; i++) {
		ts_loc[indexes[i]]->val = 0;
	}
*/
	...
	for (int i = 0; i < tx->writes_pos; i++) {
		int srv_addr = tx->w_zone[i];
		lock_entry* entry_p = &(lock_table[srv_addr][tx->writes[i]]);
		entry_p->version = next_ts[srv_addr];
...
#define TM_BEGIN												\
	{															\
		...
		{														\
			...
			tx->writeset->reset();								\
			for (int i=0; i<8; i++) {tx->touched_zones[i] = 0;}\
			for (int i=0; i<8; i++) {							\
				tx->start_time[i] = ts_vectors[tx->numa_zone]->val[i];							\
			}													\
