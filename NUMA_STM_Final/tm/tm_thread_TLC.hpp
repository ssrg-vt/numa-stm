...
FORCE_INLINE T tm_read(T* addr, Tx_Context* tx, int numa_zone)
{
	...
	if (v1 <= tx->timestamps[entry_p->id] && (v1 == v2)) {
		int r_pos = tx->reads_pos++;
		tx->reads[r_pos] = index;
		//tx->r_zone[r_pos] = numa_zone;
		return val;
	} else {
		tx->timestamps[entry_p->id] = entry_p->version;
		tm_abort(tx, 0);
	}
	return val;
}
...
FORCE_INLINE void tm_commit(Tx_Context* tx)
{
	...
	for (int i = 0; i < tx->reads_pos; i++) {
		...
		if (entry_p->version > tx->timestamps[entry_p->id]) {
			tx->timestamps[entry_p->id] = entry_p->version;
			do_abort = true;
		}
	}
	...
	//increment my timestamp
	uintptr_t end_time = ++tx->timestamp.val;

	//update versions & unlock
	for (int i = 0; i < tx->writes_pos; i++) {
		int srv_addr = 0;//tx->w_zone[i];
		lock_entry* entry_p = &(lock_table[srv_addr][tx->writes[i]]);
		entry_p->version = end_time;
		entry_p->id = tx->id;
		entry_p->lock_owner = 0;
	}
