...
//	ts_vector* my_ts = ts_vectors[0];
	unsigned long long time = get_real_time();
	for (int i=0; i<100000; i++) {
		while(!__sync_bool_compare_and_swap(&(ts_vectors[0]->val[0]), 0, 1)) {
			while(ts_vectors[0]->val[0]) pause();
		}
		for (int o=0; o<10; o++) spin64();
		ts_vectors[0]->val[0] = 0;
	}
	time = get_real_time() - time;

	printf("%d: Centralized Total Time = %llu\n", id, time);
