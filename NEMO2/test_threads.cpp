...
void* th_run(void * args)
{
    ...
    printf("my id %d and zone %d (index %d)\n", id, numa_zone, index);
    	...
    	short* accounts = accountsAll;

    	short* accounts2 = accountsAll;
...
//	for (int j=0; j < ZONES; j++) {
		accountsAll = (short*) numa_alloc_onnode(sizeof(short) * ACCOUT_NUM, 0);//malloc(sizeof(long) * ACCOUT_NUM);// create_shared_mem(j, sizeof(long) * ACCOUT_NUM, SHARED_MEM_KEY5);//createSharedMem(j);
//	}
