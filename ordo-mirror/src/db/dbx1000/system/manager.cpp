...
bool
Manager::clock_is_min_and_wait(uint64_t *t1, uint64_t *t2)
{
    /* t2 > t1 */
    int64_t diff;
    if (*t2 > *t1 + _clock_gap)
        return true;
    if (*t2 > *t1) {
        diff = int64_t(_clock_gap) - int64_t(*t1  - *t2);
...
bool
Manager::clock_is_min(uint64_t *t1, uint64_t *t2)
{
    /* t2 > t1 */
    if (*t2 > *t1 + _clock_gap)
        return true;
