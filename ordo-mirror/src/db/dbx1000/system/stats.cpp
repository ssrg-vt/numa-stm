...
void Stats::commit(uint64_t thd_id) {
    if (STATS_ENABLE) {
        ...
        _stats[thd_id]->time_validate += tmp_stats[thd_id]->time_validate;
...
void Stats::print() {
    ...
    for (uint64_t tid = 0; tid < g_thread_cnt; tid ++) {
        ...
        total_ww_abort_cnt += _stats[tid]->ww_abort_cnt;
    ...
    if (output_file != NULL) {
        ...
        fprintf(outf, "[summary] txn_cnt=%ld, total_abort_cnt=%ld, write_abort_cnt=%ld"
                ", run_time=%f, time_wait=%f, time_ts_alloc=%f"
                ", time_man=%f, time_validate=%f, time_index=%f, time_abort=%f, time_cleanup=%f, latency=%f"
                ", deadlock_cnt=%ld, cycle_detect=%ld, dl_detect_time=%f, dl_wait_time=%f"
                ", time_query=%f, debug1=%f, debug2=%f, debug3=%f, debug4=%f, debug5=%f, debug6=%f, debug7=%f\n",
                total_txn_cnt,
                total_abort_cnt,
                total_ww_abort_cnt,
                ...
                total_latency / MILLION / total_txn_cnt,
                deadlock,
                cycle_detect,
    ...
    printf("[summary] txn_cnt=%ld, total_abort_cnt=%ld, write_abort_cnt=%ld"
           ", run_time=%f, time_wait=%f, time_ts_alloc=%f"
           ", time_man=%f, time_validate=%f, time_index=%f, time_abort=%f, time_cleanup=%f, latency=%f"
           ", deadlock_cnt=%ld, cycle_detect=%ld, dl_detect_time=%f, dl_wait_time=%f"
           ", time_query=%f, debug1=%f, debug2=%f, debug3=%f, debug4=%f, debug5=%f, debug6=%f, debug7=%f\n",
