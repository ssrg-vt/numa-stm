...
#define PHYSICAL_TIMESTAMPING
#define MACHINE_MOS001

#ifndef CORES_PER_SOCKET

#ifdef MACHINE_OPTIMUS
#define CORES_PER_SOCKET 10
#elif defined(MACHINE_MOS001)
#define CORES_PER_SOCKET 15
#elif defined(MACHINE_GOLDBUG)
#define CORES_PER_SOCKET 8
#else
#ifndef KERNEL
#define CORES_PER_SOCKET sysconf(_SC_NPROCESSORS_ONLN)
#else
#define CORES_PER_SOCKET num_online_cpus()
#endif /* KERNEL */
#endif /* defining CORES_PER_SOCKET */

#endif /* CORES_PER_SOCKET */

#ifdef PHYSICAL_TIMESTAMPING
#define DO_HRL_COMBINING
//#define DO_COMBINING
#endif
...
#define COMBINER_COUNT  128
#define false 0
#define true 1

struct cmb_node {
    volatile struct cmb_node *next ____cacheline_aligned;
    long padding_0[RLU_DEFAULT_PADDING];

    volatile unsigned long clock ____cacheline_aligned;
    int wait;
    int completed;
...
struct cmb_snode {
    volatile struct cmb_snode *snext ____cacheline_aligned;
    long padding_0[RLU_DEFAULT_PADDING];

    volatile int locked ____cacheline_aligned;
    long padding_1[RLU_DEFAULT_PADDING];

    volatile struct cmb_node *node_tail ____cacheline_aligned;
    int combining_count;
...
#else
    volatile unsigned long local_clock;
    volatile unsigned long local_commit_clock;
#endif
...
#else
    volatile unsigned long writer_clock;
#endif
...
#if defined(DO_COMBINING) || defined(DO_HRL_COMBINING)
    struct cmb_node node ____cacheline_aligned;

    long padding_8[RLU_DEFAULT_PADDING];
#endif
...
#define RLU_THREAD_INIT_CORE(self, cid) rlu_thread_init_core(self, cid)
