...
static void thread_init(thread_data_t *d) {
	RLU_THREAD_INIT_CORE(d->p_rlu_td, d->core_id);
	prcu_register(d->uniq_id);
}
...
static int citrus_tree_contains(thread_data_t *d, int key) {
#ifdef IS_RLU
	return rlu_tree_contains(d->p_rlu_td, d->citrus_root, key);
#else
#ifdef IS_RCU
	return contains(d->citrus_root, key);
#endif
#endif
}

static bool citrus_tree_add(thread_data_t *d, int key) {
#ifdef IS_RLU
	return rlu_tree_add(d->p_rlu_td, d->citrus_root, key, key);
#else
#ifdef IS_RCU
	return insert(d->citrus_root, key, key);
#endif
#endif
}

static bool citrus_tree_remove(thread_data_t *d, int key) {
#ifdef IS_RLU
	return rlu_tree_remove(d->p_rlu_td, d->citrus_root, key);
#else
#ifdef IS_RCU
	return delete(d->citrus_root, key);
#endif
#endif
}
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < nb_threads; i++) {
		...
		data[i].citrus_root = citrus_root;
