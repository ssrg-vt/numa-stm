...
#ifdef NANO_EPOCHS
      if (version >= tx->start + _tinystm.clock_diff &&
          (version + _tinystm.clock_diff <= tx->end ||
           (!tx->attr.read_only && stm_wbetl_extend(tx)))) {
#else
      if (version >= tx->start &&
          (version <= tx->end ||
           (!tx->attr.read_only && stm_wbetl_extend(tx)))) {
...
# endif /* UNIT_TX */
 ...
 acquire:
#ifdef NANO_EPOCHS
  if (unlikely(version > tx->end + _tinystm.clock_diff)) {
#else
  if (unlikely(version > tx->end)) {
#endif
