...
{
#ifdef NANO_EPOCHS
        uint64_t v = read_tsc() << LOCKBIT;
        /* swap_uint64(&Self->rv, read_tscp() << LOCKBIT); */
        return v;
#else /* !NANO_EPOCHS */
#if 1
        return _GCLOCK;
...
#else /* NANO_EPOCHS */
        wv = read_tsc() << LOCKBIT;
        if (wv - Self->rv < CLOCK_DIFF) {
                int i = (CLOCK_DIFF - (wv - Self->rv)) / 10;
                while (i-- >= 0) {
                        asm volatile ("pause");
                }
                wv = read_tsc() << LOCKBIT;
        }
        swap_uint64(&Self->wv, wv);
        return Self->wv;
#endif
}
 ...
 *
 * TODO: make the frequency mask adaptive at runtime.
...
#else /* !TL2_EAGER */
__INLINE__ void
RecordStore (Log* k, volatile intptr_t* Addr, intptr_t Value, volatile vwLock* Lock)
{
        /*
 ...
 */
__INLINE__ void
SaveForRollBack (Log* k, volatile intptr_t* Addr, intptr_t Value)
{
        AVPair* e = k->put;
        if (e == NULL) {
                k->ovf++;
...
TrackLoad (Thread* Self, volatile vwLock* LockFor)
{
        ...
        k->put     = e->Next;
        e->LockFor = LockFor;
        /* Note that Value and Addr fields are undefined for tracked loads */
        return 1;
}
...
TxInitThread (Thread* t, long id)
{
        ...
        memset(t, 0, sizeof(*t));     /* Default valuee for most members */
        t->UniqID = id;
        t->rng = id + 1;
        t->xorrng[0] = t->rng;
...
TryFastUpdate (Thread* Self)
{
	Log* wr;
         ...
         * Validate that the set of valuees we've fetched from pure READ objects
         ...
         * Next we validate that the valuees we've fetched from pure READ objects
         * remain coherent.
         *
...
#ifdef TL2_EAGER
void
TxStore (Thread* Self, volatile intptr_t* addr, intptr_t value)
{
         ...
         * set the lockbit in the lockword and use a temporary AVPair* valuee.
        ...
        if ((cv & LOCKBIT) && (((AVPair*)(cv ^ LOCKBIT))->Owner == Self)) {
                /*
                 * We own this lock; update cv with the correct valuee for RecordStore.
                 */
...
TxStore (Thread* Self, volatile intptr_t* addr, intptr_t value)
{
        ...
        if (ALWAYS && LDNF(addr) == value) {
                AVPair* e;
                for (e = wr->tail; e != NULL; e = e->Prev) {
                        ASSERT(e->Addr != NULL);
                        if (e->Addr == addr) {
                                ASSERT(LockFor == e->LockFor);
                                e->Value = value; /* CCM: update associated valuee in write-set */
                                PROF_STM_WRITE_END();
                                return;
                        }
                }
                /* CCM: Not writing new valuee; convert to load */
        ...
        RecordStore(wr, addr, value, LockFor);
...
#  endif
         ...
         * writing a versioned write-lock valuee that is > Self->rv, so spinning
...
txSterilize (void* Base, size_t Length)
{
        ...
        ASSERT(Addr <= End);
#ifdef NANO_EPOCHS
        uint64_t clock = read_tsc() << LOCKBIT;
#endif
        while (Addr < End) {
 ...
 */
void
TxStoreLocal (Thread* Self, volatile intptr_t* Addr, intptr_t Value)
{
