...
void row_t::init_manager(row_t * row) {
    ...
    manager = (Row_eptmstmp *) _mm_malloc(sizeof(Row_eptmstmp), 64);
#elif CC_ALG == SILO
...
RC row_t::get_row(access_t type, txn_man * txn, row_t *& row) {
    ...
    row = txn->cur_row;
    return rc;
#elif CC_ALG == TICTOC || CC_ALG == SILO || CC_ALG == EPTMSTMP
    // like OCC, tictoc also makes a local copy for each read/write
    /* XXX: Is it really used somewhere */
    row->table = get_table();
    TsType ts_type = (type == RD)? R_REQ : P_REQ;
    rc = this->manager->access(txn, ts_type, row);
    return rc;
