...
void Row_mvcc::init(row_t * row) {
    _row = row;
    _his_len = THREAD_CNT * 2;
    _req_len = _his_len;

    _write_history = (WriteHisEntry *) _mm_malloc(sizeof(WriteHisEntry)
                                                  * _his_len, 64);
...
void Row_mvcc::buffer_req(TsType type, txn_man * txn, bool served)
{
    ...
    while (true) {
        for (uint32_t i = 0; i < _req_len; i++) {
            ...
            // in some sense, only need to keep the served read request with
            // the max ts.
...
{
    if (list == 0) {
        WriteHisEntry * temp = (WriteHisEntry *)
            _mm_malloc(sizeof(WriteHisEntry) * _his_len * 2, 64);
        for (uint32_t i = 0; i < _his_len; i++) {
        ...
        assert(list == 1);
        ReqEntry * temp = (ReqEntry *) _mm_malloc(sizeof(ReqEntry) *
                                                  _req_len * 2, 64);
        for (uint32_t i = 0; i < _req_len; i++) {
...
/* TODO: Sanidhya: Implement timing logic here */
   ...
   //pthread_mutex_lock( latch );
    ...
    if (type == R_REQ) {
        if (ts < _oldest_wts)
            ...
            {
                // exists a pending prewrite request before the current read.
                // should wait.
                rc = WAIT;
            ...
            rc = RCOK;
            // ts is between _oldest_wts and _latest_wts, should find the
            // correct version
            uint32_t the_ts = 0;
        ...
        if (ts < _latest_wts
            || ts < _max_served_rts
            || (_exists_prewrite && _prewrite_ts > ts))
            rc = Abort;
...
Row_mvcc::reserveRow(ts_t ts, txn_man * txn)
{
    ...
    if (idx == _his_len) {
        if (_his_len >= g_thread_cnt) {
            // all entries are taken. recycle the oldest version if _his_len is
            // too long already
