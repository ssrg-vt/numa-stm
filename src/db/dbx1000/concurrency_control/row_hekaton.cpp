...
void Row_hekaton::init(row_t * row) {
	...
	for (uint32_t i = 0; i < _his_len; i++) {
		_write_history[i].row = (row_t *) _mm_malloc(sizeof(row_t), 64);
        _write_history[i].row->init(MAX_TUPLE_SIZE);
    }
...
RC Row_hekaton::access(txn_man * txn, TsType type, row_t * row) {
	RC rc = RCOK;
	ts_t ts = txn->get_ts();
    /* XXX: Wrong implementation, this is not called lock-free */
...
#if USE_GLOBAL_CLOCK
            else if (!_write_history[idx].end_txn) {
                if (glob_manager->clock_is_min(&commit_ts, &_write_history[idx].end))
                    rc = RCOK;
                else
                    rc = Abort;
            }
