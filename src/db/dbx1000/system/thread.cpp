...
RC thread_t::run() {
	...
	while (true) {
		...
		if (rc == RCOK) {
            ...
            if (decode_abrt_type(rc) == ABORT_WW)
                INC_STATS(get_thd_id(), ww_abort_cnt, 1);
...
#if CC_ALG == HEKATON && USE_GLOBAL_CLOCK
            _curr_ts = get_sys_clock();
#else
