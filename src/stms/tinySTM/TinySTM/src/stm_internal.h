...
#define MAX_THREADS                     512                /* Upper bound (large enough) */
...
#ifdef NANO_EPOCHS
  volatile int clock_diff ALIGNED;
#endif
...
#ifdef NANO_EPOCHS
INLINE void burn_cycles(int diff)
{
    int i;
    if (diff >= _tinystm.clock_diff)
        return;
    for (i = 0; i < (_tinystm.clock_diff - diff) / 10; ++i)
        asm volatile ("pause":::"memory");
}

INLINE uint64_t wait_and_fetch_timestamp(uint64_t t)
{
    uint64_t temp = rdtsc();
    assert(temp >= t);
    burn_cycles(temp - t);
...
#ifdef NANO_EPOCHS
    gc_free(tx->w_set.entries, rdtsc());
#else
