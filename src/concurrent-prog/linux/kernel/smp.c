...
EXPORT_PER_CPU_SYMBOL(timestamp);
...
int smp_call_function_rdtsc_fast(smp_call_func_t func, void *info, int wait,
			   struct rdtsc_pair *rp)
{
	preempt_disable();
	smp_call_function_all_fast(cpu_online_mask, func, info, wait, rp);
	preempt_enable();

	return 0;
}
EXPORT_SYMBOL(smp_call_function_rdtsc_fast);
...
static int __init setup_tt_offset(char *str)
{
        get_option(&str, &tt_offset);
        if (tt_offset <= 0)
                tt_offset = 500;
        return 0;
}

early_param("tt_offset", setup_tt_offset);
