...
struct refcount {
	...
	struct counter_obj obj[MAX_CPU];
};
...
static void inc_value(struct refcount *r, int i)
{
	++r->obj[i].count;
}
...
static void waitup(void)
{
	...
	printf("%d cores completed %d oplog_update in %"PRIu64" cycles %lu tput %.3g\n",
