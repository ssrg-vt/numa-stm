...
#define relax() asm volatile("rep; nop" ::: "memory")
...
#define RLU_TRACE(self, fmt, ...) \
    fprintf(stderr, "[%ld][%ld][%ld]:%s:%d:%s(): " fmt, \
            self->uniq_id, self->run_counter, self->local_clock, \
			__FILE__, __LINE__, __func__, \
            __VA_ARGS__);
#endif
...
#else /* !PHYSICAL_TIMESTAMPING */
static volatile int64_t CLOCK_DIFF = 250;
static volatile unsigned long g_rlu_array[RLU_CACHE_LINE_SIZE * 64] = {0,};

#define g_rlu_commit_clock g_rlu_array[RLU_CACHE_LINE_SIZE * 2]

#ifdef DO_COMBINING
static volatile struct cmb_node *tail ____cacheline_aligned;
#endif /* DO_COMBINING */

#ifdef DO_HRL_COMBINING
static volatile struct cmb_snode *stail ____cacheline_aligned;
static volatile struct cmb_snode snodes[64] ____cacheline_aligned;
#endif /* DO_HRL_COMBINNG */

#endif /* PHYSICAL_TIMESTAMPING */
...
static inline unsigned long read_serialized_tscp(void)
{
    unsigned int a, d;
...
/*
 * The simple logic is that if my clock is greater the g_rlu_commit_clock,
 * then only I'll try to do a CAS operation. Here we can use hierarchical
 * combining like cc-sync to update the values without using any CAS
 * operation as they will be added to the list.
 */
#ifdef PHYSICAL_TIMESTAMPING

static inline int check_write_clock(volatile uint64_t *c1,
				    volatile uint64_t c2)
{
	uint64_t old_value = *c1;
	int flag = 0;
	for (;;) {
		if (flag && *c1 == old_value) {
			return *c1 >= c2;
		}
		if (*c1 >= c2 + CLOCK_DIFF)
			return 1;
			...
			int64_t diff = *c1 - c2;
			flag = 1;
			if (diff < (-1) * CLOCK_DIFF)
				return 0;
			if (diff > -1 * CLOCK_DIFF && diff < 0) {
				int loop, total_loops;
				diff *= -1;
				total_loops = (CLOCK_DIFF - diff) / 10;
				for (loop = 0; loop < total_loops; ++loop)
					relax();
				continue;
			}
			/*
			 * if 0 < c2 - c1 < CLOCK_DIFF;
			 * 	burn cpu cycle and compare
			 */
			if (diff > 0 && diff < CLOCK_DIFF) {
				int loop, total_loops;
				total_loops = (CLOCK_DIFF - diff) / 10;
				for (loop = 0; loop < total_loops; ++loop)
					relax();
...
#if defined(DO_COMBINING) || defined(DO_HRL_COMBINING)
static inline void update_clock(unsigned long clock)
{
    if (g_rlu_commit_clock < clock) {
        g_rlu_commit_clock = clock;
        MEMBARSTLD();
    }
}

static inline void update_node(unsigned long clock,
                               struct cmb_node *node)
{
    node->next = NULL;
    node->wait = true;
...
static inline int request_update(struct cmb_node **tail,
                                 struct cmb_node *node)
{
    struct cmb_node *prev;
...
static inline int wait_until_done(struct cmb_node *node)
{
    while (node->wait)
        relax();
...
static inline int should_update_clock(unsigned long clock,
                                      struct cmb_node *tail)
{
    struct cmb_node *tmp;

    if (clock <= g_rlu_commit_clock)
        return 0;
...
static inline int update_node_and_wait(unsigned long clock,
                                       struct cmb_node **tail,
                                       struct cmb_node *node)
{
    if (!should_update_clock(clock, *tail))
        return 0;

    update_node(clock, node);

    if (!request_update(tail, node)) {
        if (wait_until_done(node))
            return 0;
    }
    return 1;
}
...
#ifdef DO_COMBINING
static inline void update_commit_clock(unsigned long clock,
                                       struct cmb_node *node)
{
    struct cmb_node *next, *cur;
    int count;

    if (!update_node_and_wait(clock, (struct cmb_node **)&tail, node))
        return;

    /* I am the combiner */
    for (count = COMBINER_COUNT, cur = node; count > 0; count--, cur = next) {
        update_clock(cur->clock);
        if (unlikely(!(next = (struct cmb_node *)cur->next))) {
            if (unlikely(CAS(&tail, cur, NULL) == cur)) {
                cur->completed = true;
                cur->wait = false;
                return;
            }
            ...
            barrier();
            while (!(next = (struct cmb_node *)cur->next))
                relax();
        }
        cur->completed = true;
        cur->wait = false;
...
#elif defined(DO_HRL_COMBINING)
static inline void acquire_global_lock(struct cmb_snode *snode)
{
    struct cmb_snode *prev;

    snode->snext = NULL;
    snode->locked = false;

    prev = (struct cmb_snode *)xchg(&stail, snode);
    if (!prev)
        return;
    ...
    while (snode->locked == false)
        relax();
}

static inline void release_global_lock(struct cmb_snode *snode)
{
    struct cmb_snode *next;

    rmb();
    next = (struct cmb_snode *)snode->snext;

    if (!(next)) {
        if (CAS(&stail, snode, NULL) == snode)
            return;

        while (!(next = (struct cmb_snode *)snode->snext))
            relax();
    }
    next->locked = true;
}
...
{
    struct cmb_node *next, *cur;
    struct cmb_snode *snode = (struct cmb_snode *)&snodes[node_id];
    int count;

    if (!update_node_and_wait(clock, (struct cmb_node **)&snode->node_tail,
                              node))
        return;

    /* I am the combiner, but now I have to acquire the global lock */
    acquire_global_lock(snode);

    for (count = COMBINER_COUNT, cur = node; count > 0; count--, cur = next) {
        update_clock(cur->clock);

        if (unlikely(!(next = (struct cmb_node *)cur->next))) {
            release_global_lock(snode);
	    cur->completed = true;
	    cur->wait = false;
            if (unlikely(CAS(&snode->node_tail, cur, NULL) == cur))
                return;
...
#endif /* DO_HRL_COMBINING */

#else
static inline void update_commit_clock(unsigned long clock)
{
     for (;;) {
         unsigned long global_commit_clock = g_rlu_commit_clock;

         /* clock is already small, commit version is way ahead! */
         if (clock <= global_commit_clock)
             break;

         if (CAS(&g_rlu_commit_clock, global_commit_clock, clock) ==
             global_commit_clock)
             break;
...
#else
			self->writer_clock, p_obj_actual, p_obj_copy, obj_size);
#endif
...
#else
	rmb();
	self->local_clock = read_tsc();
	wmb();
	self->local_commit_clock = self->local_clock + 1; //g_rlu_commit_clock;
#endif
}
...
#else
            if (write_clock) {
                /* if (g_rlu_threads[th_id]->local_clock >= write_clock) { */
		   if (check_write_clock(&g_rlu_threads[th_id]->local_clock,
					 write_clock)) {
#endif
					self->q_threads[th_id].is_wait = 0;
					break;
...
#else
                    g_rlu_threads[th_id]->local_clock,
#endif
...
#else
	q_iters = rlu_wait_for_quiescence(self, self->writer_clock);
#endif
...
#else
	/* self->writer_clock = read_serialized_tsc(); */
	swap_uint64(&self->writer_clock, read_tscp());
#endif
...
#else
	self->writer_clock = MAX_VERSION;
#endif

#ifndef PHYSICAL_TIMESTAMPING
	FETCH_AND_ADD(&g_rlu_commit_version, 1);
#else
    /* need to update it using cas opearation */
#ifdef DO_COMBINING
    /* update_commit_clock(writer_clock, &self->node); */
#elif defined(DO_HRL_COMBINING)
    /* update_commit_clock(writer_clock, &self->node, self->node_id); */
#else
    /* update_commit_clock(writer_clock); */
#endif /* DO_COMBINING */
...
#if defined(PHYSICAL_TIMESTAMPING) && defined(DO_COMBINING)
    tail = NULL;
#endif
}
...
void rlu_thread_init_core(rlu_thread_data_t *self, int core_id)
{
    rlu_thread_init(self);
    self->core_id = core_id;
    self->node_id = core_id / CORES_PER_SOCKET;
}
...
void rlu_reader_lock(rlu_thread_data_t *self) {
	...
	if (((self->local_version - self->local_commit_version) == 0) &&
#else
	if (((self->local_clock - self->local_commit_clock) == 0) &&
#endif
...
#else
        	(g_rlu_threads[th_id]->writer_clock <= self->local_clock)) {
#endif
...
#else
        self->local_clock, p_obj, GET_COPY(p_obj), GET_THREAD_ID(p_obj));
#endif
